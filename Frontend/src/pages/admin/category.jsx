...
const Category = () => {
  ...
  const onSubmit = async (data) => {
    try {
      if (editingId) {
        await axiosInstance.put(`${endpoints.createCategory}/${editingId}`, data);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        await axiosInstance.delete(`${endpoints.createCategory}/${id}`);
            ...
            <table className="min-w-full divide-y divide-[#333] border border-[#D4AF37]/20 rounded-md">
              <thead className="bg-gradient-to-r from-[#1a1a1a] to-[#111]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-bold text-white">Category</th>
                  <th className="px-4 py-3 text-left text-sm font-bold text-white">Description</th>
              ...
              <tbody className="divide-y divide-[#2a2a2a]">
                {categories.map((category, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-white font-medium">{category.category}</td>
                    <td className="px-4 py-3 text-sm text-[#CCCCCC]">{category.descriptions}</td>
                ...
                {categories.length === 0 && (
                  <tr>
                    <td colSpan="4" className="text-center text-[#888] py-6 text-sm italic">
                      No categories found.
                    </td>
                  </tr>
                )}
